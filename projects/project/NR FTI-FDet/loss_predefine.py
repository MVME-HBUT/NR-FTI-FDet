...
## F1 Loss
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        if self.head_type == 'CenterNet':
            ...
            bs, k, hw = src_boxes.shape
            src_boxes = src_boxes.permute(0, 2, 1).reshape(bs, hw, k)
...
class MinCostMatcher(nn.Module):
    ...
    def __init__(self, cfg, cost_class: float = 1, cost_bbox: float = 1, cost_giou: float = 1):
        ...
        self.head_type = cfg.MODEL.OneNet.HEAD
        assert self.head_type == "CenterNet" or self.head_type == "RetinaNet" or self.head_type == "FCOS"
        self.pre_define = cfg.MODEL.OneNet.PRE_DEFINE
        self.object_sizes_of_interest = torch.tensor(cfg.MODEL.OneNet.OBJECT_SIZES_OF_INTEREST)
    ...
    def compute_cost_fpn(self, cost_bbox, tgt_bbox, out_bbox, fpn_level):
        ...
        '''
        object_sizes_of_interest = self.object_sizes_of_interest[fpn_level.long()].to(cost_bbox.device)
    ...
    def forward(self, outputs, targets):
        ...
        if self.head_type == 'CenterNet':
            ...
            if self.pre_define:
                batch_out_bbox = outputs["anchors"].permute(0, 2, 1)  # [batch_size, num_queries, 4]            
            else:
                batch_out_bbox = outputs["pred_boxes"].permute(0, 2, 1)  # [batch_size, num_queries, 4]
            ...
            if self.pre_define:
                pre_locations = outputs["locations"]
                pre_boxes = torch.cat([pre_locations, pre_locations], dim=1)
                batch_out_bbox = pre_boxes.permute(0, 2, 1) # [batch_size, num_queries, 4]
...
#                 indices.append((torch.as_tensor([]).to(batch_out_prob), torch.as_tensor([]).to(batch_out_prob)))
            ...
            if self.pre_define:             
                if self.head_type == 'CenterNet':
                    ...
                    # comparison center distance
                    cost_bbox = torch.cdist(out_bbox_[:,:2]+out_bbox_[:,2:], tgt_bbox_[:,:2]+tgt_bbox_[:,2:], p=1)
                    cost_fpn = self.compute_cost_fpn(cost_bbox, tgt_bbox, out_bbox, batch_fpn_levels[i][0])
                    C = self.cost_bbox * cost_bbox + self.cost_class * cost_class + cost_fpn
            else:
                # Compute the L1 cost between boxes
                cost_bbox = torch.cdist(out_bbox_, tgt_bbox_, p=1)                    
                # Compute the giou cost betwen boxes
